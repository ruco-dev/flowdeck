...
| `invoices/` | `mark-issued`, `mark-paid`, `void`, **`client-report`**, **`moloni-export`**, **`draft-email`** |
...
**Whitelist, by design.** The script reads **only** the header fields the document type declares plus the item table's client columns (`# / Description / Qty / Unit Price / Total`). It never reads the `Hash` column, the `## Notes` section, or the `## Updates` section — so internal data (hours est-vs-real, ticket refs, `Billing Ref`, hash-remint history, pricing caveats, approval deliberation) cannot leak, even if someone adds a new internal field later. A human still reviews the HTML before it reaches the client — this is draft-not-send.

---

## moloni-export

From an `INVOICE.md`, produce a client-facing **proforma** and a **Moloni import payload**.

**Trigger:** `- [ ] moloni-export` on an `invoices` card, or on a specific invoice's `TODO.md`.

Run: `node .flowdeck/.creamdeck/_scripts/moloni-export.js <invoice-id-or-folder> [--vat <rate>] [--lang <code>]` from the project root. Writes, next to `INVOICE.md`:
- `invoice-proforma.html` — subtotal + VAT + total, watermarked **not a fiscal invoice**. A preview for the client, not a legal document.
- `invoice-moloni.json` — a `documents/invoices/insert`-shaped payload.

**PT / SAF-T:** the certified invoice must be issued **by** Moloni. This action never emits a fiscal document; the JSON is the *input* that creates the real invoice inside Moloni, where it is legally issued.

Line items are **net**; VAT is applied at `--vat` (default `23`). Account-specific IDs — `company_id`, `customer_id`, `document_set_id`, `tax_id`, `product_id` — cannot be derived from a card and are emitted `null` under `_requires`; fill them from your Moloni account before POSTing. No API call is made; verify the field names against current Moloni API docs before wiring one.
...
2. Ensure a client-safe artifact exists — run `client-report` (or `moloni-export` for an invoice) first if it doesn't.
