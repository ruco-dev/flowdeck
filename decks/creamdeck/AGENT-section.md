...
**Authorization vs. delivery vs. invoicing** (ADR-0002): three separate questions, three separate mechanisms — never infer one from another. Priced-and-approved = the item's hash exists. Work-done = the ticket's own `Stage` (Resolved/Closed), unrelated to billing. Safe-to-invoice = both together, checked only at `generate-invoice`, which now skips (surfaces under `## HUMAN`) any hashed item whose linked ticket isn't yet Resolved/Closed. `link-proposal-item` never checks `Stage` — linking a brand-new, unstarted ticket is normal, not a shortcut.
