...
`flowdeck install`/`update creamdeck` installs `report.js`, `html.js`, `approve-proposal.js`, `export-invoice.js`, `client-report.js`, and `moloni-export.js` straight into `.flowdeck/.creamdeck/_scripts/` — `manifest.scripts` routes there directly because `installRoot` is set, no separate staging copy involved. See "Reports & exports" below for the runtime paths.
...
Three actions turn a billing document into something the client can receive. All three are **draft-not-send**: they write a file next to the source document; a human reviews and sends it. See `ACTIONS.md` for the full action reference.

- `_scripts/client-report.js <doc-id> [--lang <code>]` — renders `<doc-folder>/client-report.html`, a **client-safe projection** of a proposal / request note / invoice. Action: `client-report` (on the `proposals`, `request-notes`, and `invoices` cards). It is a **whitelist**: only the header fields the type declares plus the item table's client columns (`# / Description / Qty / Unit Price / Total`) are read. The `Hash` column, `## Notes`, and `## Updates` — where internal data lives (hours est-vs-real, ticket refs, `Billing Ref`, hash-remint history, pricing caveats, approval deliberation) — are **never read**, so adding an internal field to a source doc can never leak it. A human is still the final gate before sending.
- `_scripts/moloni-export.js <invoice-id> [--vat <rate>] [--lang <code>]` — from an `INVOICE.md`, writes `invoice-proforma.html` (a preview watermarked **not a fiscal invoice**) **and** `invoice-moloni.json` (a `documents/invoices/insert`-shaped payload). Action: `moloni-export` (on the `invoices` card). PT/SAF-T: the certified invoice must be issued **by** Moloni — this script never emits a fiscal document; the JSON is the *input* that creates the real invoice inside Moloni. Account-specific IDs (`company_id`, `customer_id`, `document_set_id`, `tax_id`, `product_id`) are emitted `null` under `_requires`; line items are **net**, VAT is applied at `--vat` (default 23). No API call is made — verify the field names against current Moloni API docs before wiring one.
- `draft-email` (agent-performed, on any billing card) — composes a cover email from the document into `<doc-folder>/email-draft/EMAIL.md` (from `_energy-cards/EMAIL-DRAFT.md.template`), in the contact's language, attaching the **client-safe** artifact (`client-report.html` / `invoice-proforma.html`), never the raw `.md`. Never dispatched by creamdeck — review by hand or hand to emaildeck's `draft-email` to create a Gmail draft.
